...
public class CapacityServiceImpl implements CapacityService {
    ...
    @Override
    public Page<Capacity> adminGetListCapacity(String name, int page) {
        page--;
        if (page <= 0) {
            page = 0;
        }
        Pageable pageable = PageRequest.of(page, Contant.LIMIT_CAPCITY , Sort.by("createdDate").descending());
        return capacityRepository.findByNameContaining( name,  pageable);
    }
}
